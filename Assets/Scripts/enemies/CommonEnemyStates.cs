...
namespace enemies
{
    public class CommonIdleState<T> : State<T> where T : MonoBehaviour, IEnemy
    {
    ...
    public class CommonChasePlayerState<T> : State<T> where T : MonoBehaviour, IEnemy
    {
    ...
    public class CommonPatrolState<T> : State<T> where T : MonoBehaviour, IEnemy
    {
        public bool PlayerNearby { get; private set; }
        private Boolean? hasRigidbody = null;
    ...
    public class CommonAttackState<T> : State<T> where T : MonoBehaviour, IEnemy
    {
    ...
    public class CommonTakeDamageState<T> : State<T> where T : MonoBehaviour, IEnemy
    {
        ...
        public override void Enter()
        {
            ...
            owner.TakeDamage();
            if (owner.GetLives() <= 0 && stateMachine.HasState<CommonDeathState<T>>())
            {
                stateMachine.ChangeState<CommonDeathState<T>>();
                return;
            }

            if (stateMachine.PreviousState is CommonTakeDamageState<T> previousTakeDamageState)
            {
    ...
    public class CommonDeathState<T> : State<T> where T : MonoBehaviour, IEnemy
    {
        public override void Enter()
        {
            AudioSource audioSource = owner.GetAudioSource();
